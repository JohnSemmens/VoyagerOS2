...
void sim_vessel::update()
{
	...
	if (update_time_ms < 10000) // < 10seconds
	{
		...
		Heading = wrap_360(Heading - HeadingChange + RandomHeadingComponent);
		...
		if (abs(windAngle) < 20)
		{
			...
			{ //port tack
				Heading = wrap_360(Heading + (20 + windAngle)); // increase heading, on  if too close to wind
			}
			else
			{ // Starboard tack
				Heading = wrap_360(Heading + (-20 + windAngle)); // decrease heading
			}
		}
