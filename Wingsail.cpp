...
void CheckWingSailServo(void)
{
	...
	long TimeSinceLastCommand = (millis() - WingSail.LastCommandTime) / 1000;
	long TimeSinceLastRequest = (millis() - WingSail.LastRequestTime) / 1000;
	long TimeSinceLastReponse = (millis() - WingSail.LastResponseTime) / 1000;

	// if a command has been recently sent AND we haven't checked in the laat 60 seconds then check again
	if (TimeSinceLastCommand > 5 && TimeSinceLastRequest > 60)
	{
	...
	// then send anopthe command
	if (TimeSinceLastReponse > 5 && TimeSinceLastCommand > 60 && (WingSail.Servo_microseconds != WingSail.Servo_microseconds_reponse))
	{
		// send message to Wingsail servo
		QueueMessage(TelMessageType::SetWing);
...
{
	QueueMessage(TelMessageType::POW);
}