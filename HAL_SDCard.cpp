...
void SD_Logging_1s()
{
	...
	// V1.5 1/11/2021 removed mask becuase it wasn't used.
...
void SD_Logging_1m()
{
	...
	LogFile.print(Configuration.SDCardLogDelimiter);
	LogFile.print((int)WingAngleSensor.WingSailAngleSensorPort.temperature);
	LogFile.print(Configuration.SDCardLogDelimiter);
...
//	bool Status = gps.GPS_TimeIs_Valid();
//
...
void SD_Logging_Event_Wingsail_Monitor(String Description)
{
	...
	LogFile.print(Configuration.SDCardLogDelimiter);
	LogFile.print(WingSail.LastResponseTime);
	LogFile.print(Configuration.SDCardLogDelimiter);
