...
void NavigationUpdate_FastData(void)
{
	...
	// V1.4 16/3/2022 reinstated RawHeadingError. there are still errors due to local magnetic anomalies.
	//					.. removed again
	// V1.5 20/3/2022 Apply a simple cardinal correction to the heading, using values stored in config.

	NavData.HDG_Mag = int(wrap_360(imu.Heading + Configuration.MagnetVariation + Configuration.CompassOffsetAngle));

	NavData.HDG_CPC = CompassDeviationCalc(NavData.HDG_Mag); // Apply a simple cardinal point correction to the heading, using values stored in config.

	// calculate and error value from the GPS and dampen it using low pass filter
	int RawHeadingError=0;

	// ****** the compass is much better now. Lets leave this for a while
	// ******* consider setting the threshold using Config value
	// ****** consider adding an "age"factor to the COG value. The problem is that you may get one reading, and it remembers it, and then it becomes
	// ****** useless or worse than useless.
	// -- reinstated RawHeadingError 16/3/2022. there are still errors due to local magnetic anomalies. 

	//if (NavData.SOG_Avg > 0.5) // if the SOG is reasonable then assume the COG. Now SOG_avg; was SOG, and it was given spurious results.
	//{
	...
	{
		NavData.HDG = int(wrap_360(NavData.HDG_Mag - NavData.HDG_Err - NavData.HDG_CPC));
	}
	else
	{
...
void UpdateTurnHeadingV2(void)
{
	...
	// if there's a change of sign of the calculated AWA  AND  the mode is to Gybe and we are already "initialised"
	// then COMMENCE
	if ( ((CurrentAWA * PreviousAWA) < 0) && (Configuration.TackingMethod == ManoeuvreType::mtGybe) && TurnHeadingInitialised)
	{
